...
#define PIN_MINIKIT_TDO (15)
#define PIN_MINIKIT_SVP (36)

#define PIN_LCD_BL   PIN_MINIKIT_SVP
#define PIN_LCD_DIN  (23)
#define PIN_LCD_DOUT (19)
...
#define PIN_LCD1_RST PIN_MINIKIT_TDO

// ESP32でLovyanGFXを独自設定で利用する場合の設定例

/// 独自の設定を行うクラスを、LGFX_Deviceから派生して作成します。
class LGFX_MiniKit_GC9A01_0 : public lgfx::LGFX_Device {
/*
...
//*/
    ...
    { // 表示パネル制御の設定を行います。
      ...
      cfg.pin_cs           =  PIN_LCD0_CS;  // CSが接続されているピン番号   (-1 = disable)
      cfg.pin_rst          =  PIN_LCD0_RST;  // RSTが接続されているピン番号  (-1 = disable)
...
/// 独自の設定を行うクラスを、LGFX_Deviceから派生して作成します。
class LGFX_MiniKit_GC9A01_1 : public lgfx::LGFX_Device {
/*
 クラス名は"LGFX"から別の名前に変更しても構いません。
 AUTODETECTと併用する場合は"LGFX"は使用されているため、LGFX以外の名前に変更してください。
 また、複数枚のパネルを同時使用する場合もそれぞれに異なる名前を付けてください。
...
// SPIバスの設定
      ...
      cfg.use_lock   = true;        // トランザクションロックを使用する場合はtrueを設定
      cfg.dma_channel = 1;          // Set the DMA channel (1 or 2. 0=disable)   使用するDMAチャンネルを設定 (0=DMA不使用)
      cfg.pin_sclk = PIN_LCD_CLK;            // SPIのSCLKピン番号を設定
      cfg.pin_mosi = PIN_LCD_DIN;            // SPIのMOSIピン番号を設定
      cfg.pin_miso = -1;            // SPIのMISOピン番号を設定 (-1 = disable)
...
//*/
    ...
    { // 表示パネル制御の設定を行います。
      ...
      cfg.pin_cs           =    PIN_LCD1_CS;  // CSが接続されているピン番号   (-1 = disable)
      cfg.pin_rst          =    PIN_LCD1_RST;  // RSTが接続されているピン番号  (-1 = disable)
      cfg.pin_busy         =    -1;  // BUSYが接続されているピン番号 (-1 = disable)
